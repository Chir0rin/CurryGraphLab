...
def run(initial: dict, resume=False, approve=False, max_steps=40):
    ...
    while frontier and step < max_steps:
        ...
        updates, failed = [], set()
        for n in sorted(ready):                                       # 本来は並列。ここは順に呼ぶが結果は同時に合流
            for attempt in range(RETRIES + 1):
                try:
                    if _fail_left.get(n, 0) > 0:
                        _fail_left[n] -= 1; raise RuntimeError(f"{n} 故障(注入)")
                    updates.append(NODES[n](state)); break
                except Exception as exc:
                    print(f"  ⚠ {n} 失敗 {attempt + 1}/{RETRIES + 1}: {exc}")
                    if attempt == RETRIES:
                        failed.add(n)
        if failed:
            save(step, frontier, done, state)
            print(f"\n✖ {sorted(failed)} が再試行上限に達した。チェックポイント保存、手動介入待ち(frontier は保持)。")
            return state
        ready -= failed
...
if __name__ == "__main__":
    ...
    if "--approve" in args and os.path.exists(CHECKPOINT):   # 人の入力を state に載せて再開(interrupt の返答)
